...
private:
    //F1 and F2
    static const uint8_t visemeCount = 9;
    ...
    Vector2D visDD = Vector2D(500.0f, 700.0f);
        ...
        Vector2D* coordinates[visemeCount] = { &visEE, &visAE, &visUH, &visAR, &visER, &visAH, &visOO, &visIH, &visDD };
    ...
    float visRatioIH = 0.0f;
    float visRatioDD = 0.0f;


    float* visRatios[visemeCount] = { &visRatioEE, &visRatioAE, &visRatioUH, &visRatioAR, &visRatioER, &visRatioAH, &visRatioOO, &visRatioIH, &visRatioDD };
   ...
   KalmanFilter<10> peakSmoothing = KalmanFilter<10>(0.1f);
    bool peaksBinary[peakCount]; //new
